...
public class Persistenta {
    private static Persistenta servicii = null;
    private Persistenta() {}

    public static Persistenta getInstanta()
    {
        if (servicii == null)
            servicii = new Persistenta();

        return servicii;
    }

    public static <genClass> List<genClass> citire(genClass obj, Serviciu S){
        List<genClass> genClassArrayList = new ArrayList<>();
        if(obj instanceof Facultate){
            try (BufferedReader bufferedReader = new BufferedReader(new FileReader("facultati.csv"))) {
                ...
                while ((currentLine = bufferedReader.readLine()) != null) {
                    ...
                    genClassArrayList.add((genClass) facultate);
                }
            } catch (IOException e) {
                System.out.println("Could not read data from file: " + e.getMessage());
                return null;
                ...
                while ((currentLine = bufferedReader.readLine()) != null) {
                    ...
                    for(Facultate facultate : S.getArrFacultati()) {
                        if(Integer.parseInt(dataFieldsStud[0]) == facultate.getIndex()) {
                            ...
                            genClassArrayList.add((genClass) student);
                            break;
                        }
                    }
                }
    ...
    public static <genClass> void scriere(genClass obj, Serviciu S){
        List<Facultate> facultati = new ArrayList<>(S.getArrFacultati());
        if(obj instanceof Facultate){
            ...
            try (BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter("studenti.csv"))) {
                List<Student> stud = new ArrayList<>(S.getArrStudenti());
